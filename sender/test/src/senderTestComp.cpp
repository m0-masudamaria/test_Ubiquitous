...
{
  senderTestInit(manager);
  senderInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("senderTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("senderTest0");
  if (comp == nullptr)
  {
  ...
  senderTest* testcomp = dynamic_cast<senderTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
